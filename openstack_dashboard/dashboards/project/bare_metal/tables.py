...
class TogglePause(tables.BatchAction):
    ...
    def allowed(self, request, instance=None):
        if not api.ironic.extension_supported('AdminActions',
                                            request):
            return False
        self.paused = False
        if not instance:
    ...
    def action(self, request, obj_id):
        if self.paused:
            api.ironic.server_unpause(request, obj_id)
            self.current_past_action = UNPAUSE
        else:
            api.ironic.server_pause(request, obj_id)
...
class ToggleSuspend(tables.BatchAction):
    ...
    def action(self, request, obj_id):
        if self.suspended:
            api.ironic.server_resume(request, obj_id)
            self.current_past_action = RESUME
        else:
            api.ironic.server_suspend(request, obj_id)
...
class LaunchLink(tables.LinkAction):
    ...
    def allowed(self, request, datum):
        try:
            limits = api.ironic.tenant_absolute_limits(request, reserved=True)
...
class ConfirmResize(tables.Action):
    ...
    def single(self, table, request, instance):
        api.ironic.server_confirm_resize(request, instance)
...
class RevertResize(tables.Action):
    ...
    def single(self, table, request, instance):
        api.ironic.server_revert_resize(request, instance)
...
#        instance.full_flavor = api.ironic.flavor_get(request,
#                                                   instance.flavor["id"])
